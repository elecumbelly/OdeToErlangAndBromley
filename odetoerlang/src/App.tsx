  ...
  return (
    <div className="min-h-screen bg-bg-base">
      ...
      <header className="bg-bg-surface border-b border-border-subtle">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            {/* Mode Toggle & Status */}
            <div className="flex items-center space-x-3 sm:space-x-4">
              <div className="bg-bg-elevated border border-border-subtle rounded-full p-1 flex items-center shadow-inner text-2xs">
                <button
                  onClick={enterSimple}
                  className={`px-3 py-1 rounded-full font-semibold transition-colors ${mode === 'simple' ? 'bg-cyan text-bg-base shadow-glow-cyan' : 'text-text-secondary hover:text-text-primary'}`}
                >
                  Simple
                </button>
                <button
                  onClick={() => enterAdvanced('calculator')}
                  className={`px-3 py-1 rounded-full font-semibold transition-colors ${mode === 'advanced' ? 'bg-bg-surface border border-border-subtle text-text-primary' : 'text-text-secondary hover:text-text-primary'}`}
                >
                  Advanced
                </button>
              </div>
              ...
              {mode === 'advanced' && <MathModelSettings />}
              <ThemeToggle />
            </div>
          </div>
        </div>
      ...
      <main id="main-content" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {mode === 'simple' ? (
          <SimpleLanding onOpenAdvanced={() => enterAdvanced('calculator')} />
        ) : (
          <>
            {/* Tab Content */}
