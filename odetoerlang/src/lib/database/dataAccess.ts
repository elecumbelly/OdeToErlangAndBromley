 ...
 */
function buildUpdateClause<T extends object>(
  updates: Partial<T>,
...
/**
 * Get all scenarios (excludes soft-deleted by default).
 */
...
/**
 * Get a single scenario by ID (excludes soft-deleted by default).
 */
...
/**
 * Soft-delete a scenario. Marks the row's deleted_at timestamp; SELECTs
 * filter on `deleted_at IS NULL` by default. Forecasts referencing this
 * scenario remain via their FK (no CASCADE on soft delete).
 */
...
/**
 * Get calendar events within a date range (excludes soft-deleted).
 */
...
/**
 * Soft-delete a calendar event. Sets deleted_at so the row is excluded from
 * getCalendarEvents but remains for audit / undo. EventAssignments rows
 * referencing it are unaffected (no CASCADE on soft delete).
 */
export function deleteCalendarEvent(id: number): void {
  const db = getDatabase();
  db.run('UPDATE CalendarEvents SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND deleted_at IS NULL', [id]);
  saveDatabase();
}
...
/**
 * Soft-delete a schedule plan by setting its status to 'Archived'. The
 * column already exists from v3; the queries above filter on it. Hard
 * deletes were destroying linked CoverageRequirements / Shifts via CASCADE,
